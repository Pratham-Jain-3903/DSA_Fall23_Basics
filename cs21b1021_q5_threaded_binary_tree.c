...
struct node *insert(struct node *root, int new_age, char *new_name, char *new_department, char *new_gender)
{
    ...
    if (found)
        printf("\n the name: %s exists in the database", ptr->name);
...
struct node *del(struct node *root, int farewell_age, char *farewell_name)
{
    ...
    if (found == 0)
    {
        printf("\n no one by the name: %s is present in database", farewell_name);
    }
...
;
            ...
            printf(" please insert professor age , please note that program searches by professor name only:\nage: ");
            scanf("%d", &professor_age);

            count--;

            root = del(root, farewell_age, farewell_name);
